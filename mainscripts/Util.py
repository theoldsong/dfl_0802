...
def restore_faceset_metadata_folder(input_path):
    ...
    metadata_filepath = input_path / 'meta.dat'
    io.log_info (f"还原附加信息，从 {str(metadata_filepath)}.\r\n")
...
def recover_original_aligned_filename(input_path):
    ...
    for file in io.progress_bar_generator( files, "命名中", leave=False ):
        fs, _, _, _ = file
    ...
    for file in io.progress_bar_generator( files, "命名中" ):
        fs, fd, _, _ = file
