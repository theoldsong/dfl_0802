    ...
    def on_initialize(self):
        ...
        class Encoder(nn.ModelBase):
            def on_build(self, in_ch, base_ch):    
                ...
                self.dense01 = nn.Dense( 64*64*base_ch, 16 )
                self.dense02 = nn.Dense( 16, 64 * 64 * base_ch )


                self.conv11 = ConvBlock(base_ch, base_ch*2)
                self.conv12 = ConvBlock(base_ch*2, base_ch*2)
                #self.bp1 = nn.BlurPool (filt_size=3)
                self.dense11 = nn.Dense( 32 * 32 * base_ch*2, 16 )
                self.dense12 = nn.Dense( 16, 32 * 32 * base_ch*2 )
                ...
                self.dense21 = nn.Dense( 16 * 16 * base_ch*4, 16 )
                self.dense22 = nn.Dense( 16, 16 * 16 * base_ch*4 )
                ...
                self.dense31 = nn.Dense( 8 * 8 * base_ch*8, 16 )
                self.dense32 = nn.Dense( 16, 8 * 8 * base_ch*8 )
                ...
                self.dense41 = nn.Dense( 4 * 4 * base_ch*8, 16 )
                self.dense42 = nn.Dense( 16, 4 * 4 * base_ch*8 )
            ...
            def forward(self, inp):
                ...
                x = self.conv01(x)
                x = self.conv02(x)
                x = x0 = nn.max_pool(x)#self.bp0(x)
                
                x0 = nn.flatten(x0)
                x0 = self.dense01(x0)
                x0 = self.dense02(x0)
                x0 = nn.reshape_4D (x0, 64, 64, self.base_ch)

                x = self.conv11(x)
                x = self.conv12(x)
                x = x1 = nn.max_pool(x)#self.bp1(x)
                
                x1 = nn.flatten(x1)
                x1 = self.dense11(x1)
                x1 = self.dense12(x1)
                x1 = nn.reshape_4D (x1, 32, 32, self.base_ch*2)

                x = self.conv21(x)
                x = self.conv22(x)
                x = self.conv23(x)
                x = x2 = nn.max_pool(x)#self.bp2(x)
                
                x2 = nn.flatten(x2)
                x2 = self.dense21(x2)
                x2 = self.dense22(x2)
                x2 = nn.reshape_4D (x2, 16, 16, self.base_ch*4)
                
                x = self.conv31(x)
                x = self.conv32(x)
                x = self.conv33(x)
                x = x3 = nn.max_pool(x)#self.bp3(x)
                
                x3 = nn.flatten(x3)
                x3 = self.dense31(x3)
                x3 = self.dense32(x3)
                x3 = nn.reshape_4D (x3, 8, 8, self.base_ch*8)

                x = self.conv41(x)
                x = self.conv42(x)
                x = self.conv43(x)
                x = x4 = nn.max_pool(x)#self.bp4(x)
                
                
                x4 = nn.flatten(x4)
                x4 = self.dense41(x4)
                x4 = self.dense42(x4)
                x4 = nn.reshape_4D (x4, 4, 4, self.base_ch*8)
        ...
        class Decoder(nn.ModelBase):
            def on_build(self, base_ch ):               

                self.up4 = UpConvBlock (base_ch*16, base_ch*8)
                self.uconv43 = ConvBlock(base_ch*8, base_ch*8)
                self.uconv42 = ConvBlock(base_ch*8, base_ch*8)
                self.uconv41 = ConvBlock(base_ch*8, base_ch*8)

                self.up3 = UpConvBlock (base_ch*16, base_ch*8)
                self.uconv33 = ConvBlock(base_ch*8, base_ch*8)
                self.uconv32 = ConvBlock(base_ch*8, base_ch*8)
                self.uconv31 = ConvBlock(base_ch*8, base_ch*8)

                self.up2 = UpConvBlock (base_ch*12, base_ch*4)
                self.uconv23 = ConvBlock(base_ch*4, base_ch*4)
                self.uconv22 = ConvBlock(base_ch*4, base_ch*4)
                self.uconv21 = ConvBlock(base_ch*4, base_ch*4)

                self.up1 = UpConvBlock (base_ch*6, base_ch*2)
                self.uconv12 = ConvBlock(base_ch*2, base_ch*2)
                self.uconv11 = ConvBlock(base_ch*2, base_ch*2)

                self.up0 = UpConvBlock (base_ch*3, base_ch)
                self.uconv02 = ConvBlock(base_ch*1, base_ch)
                self.uconv01 = ConvBlock(base_ch, base_ch)
                self.out_conv = nn.Conv2D (base_ch, 3, kernel_size=3, padding='SAME')

                self.mup4 = UpConvBlock (base_ch*8, base_ch*4)
                self.mup3 = UpConvBlock (base_ch*4, base_ch*2)
                self.mup2 = UpConvBlock (base_ch*2, base_ch*1)
                self.mup1 = UpConvBlock (base_ch*1, base_ch//2)
                self.mup0 = UpConvBlock (base_ch//2, base_ch//4) 
                self.out_convm = nn.Conv2D( base_ch //4, 1, kernel_size=1, padding='SAME')

            def forward(self, inp):
                x0,x1,x2,x3,x4,z = inp
                x = self.up4(tf.concat([z,x4],axis=nn.conv2d_ch_axis))
                x = self.uconv43(x)
                x = self.uconv42(x)
                x = self.uconv41(x)

                x = self.up3(tf.concat([x,x3],axis=nn.conv2d_ch_axis))
                x = self.uconv33(x)
                x = self.uconv32(x)
                x = self.uconv31(x)

                x = self.up2(tf.concat([x,x2],axis=nn.conv2d_ch_axis))
                x = self.uconv23(x)
                x = self.uconv22(x)
                x = self.uconv21(x)

                x = self.up1(tf.concat([x,x1],axis=nn.conv2d_ch_axis))
                x = self.uconv12(x)
                x = self.uconv11(x)

                x = self.up0(tf.concat([x,x0],axis=nn.conv2d_ch_axis))
                x = self.uconv02(x)
                x = self.uconv01(x)
        ...
        with tf.device (models_opt_device):
            if 'df' in archi:
                self.encoder = Encoder(input_ch, e_dims, name='encoder')
                #self.inter = Inter (in_ch=lowest_dense_res*lowest_dense_res*e_dims*8, ae_ch=ae_dims, ae_out_ch=ae_dims, name='inter')  
                self.decoder_src = Decoder(e_dims, name='decoder_src')
                self.decoder_dst = Decoder(e_dims, name='decoder_dst')
                ...
                if self.is_training:
                    if self.options['true_face_power'] != 0:
                        self.code_discriminator = nn.CodeDiscriminator(ae_dims, code_res=Inter.get_code_res()*2, name='dis' )
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi:
                        ...
                        gpu_src_x0, gpu_src_x1, gpu_src_x2, gpu_src_x3, gpu_src_x4, gpu_src_z= self.encoder(gpu_warped_src)
                        gpu_dst_x0, gpu_dst_x1, gpu_dst_x2, gpu_dst_x3, gpu_dst_x4, gpu_dst_z= self.encoder(gpu_warped_dst)
                        
                        #gpu_src_z     = self.inter(gpu_src_x3)
                        #gpu_dst_z     = self.inter(gpu_dst_x3)
                        
                        gpu_pred_src_src, gpu_pred_src_srcm = self.decoder_src( [gpu_src_x0, gpu_src_x1, gpu_src_x2, gpu_src_x3, gpu_src_x4, gpu_src_z] )
                        gpu_pred_dst_dst, gpu_pred_dst_dstm = self.decoder_dst( [gpu_dst_x0, gpu_dst_x1, gpu_dst_x2, gpu_dst_x3, gpu_dst_x4, gpu_dst_z] )
                        gpu_pred_src_dst, gpu_pred_src_dstm = self.decoder_src( [gpu_dst_x0, gpu_dst_x1, gpu_dst_x2, gpu_dst_x3, gpu_dst_x4, gpu_dst_z] )
