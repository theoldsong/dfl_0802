...
class TESTModel(ModelBase):
    ...
    def on_initialize(self):
        ...
        input_nc = 3
        output_nc = 3
        prev_bgr_shape = (prev_stage_resolution, prev_stage_resolution, output_nc)
        ...
        with tf.device ('/CPU:0'):
            ...
            self.prev_warped_src = tf.placeholder (tf.float32, (None,)+prev_bgr_shape)
            self.warped_src = tf.placeholder (tf.float32, (None,)+bgr_shape)
            self.prev_warped_dst = tf.placeholder (tf.float32, (None,)+prev_bgr_shape)
    ...
    def onGetPreview(self, samples):
        ...
        S, D, SS, DD, DDM, SD, SDM = [ np.clip(x, 0.0, 1.0) for x in ([target_src,target_dst] + self.AE_view (prev_target_src, target_src, prev_target_dst, target_dst ) ) ]
