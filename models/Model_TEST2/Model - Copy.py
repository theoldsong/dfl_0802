    ...
    def on_initialize(self):
        ...
        with tf.device (models_opt_device):
            ...
            if self.is_training:
                ...
                if self.options['true_face_training']:
                    self.D_opt = nn.RMSprop(lr=lr, lr_dropout=lr_dropout, clipnorm=clipnorm, name='D_opt')
        ...
        if self.is_training:
            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if self.options['true_face_training']:
                        ...
                        gpu_D_loss_gvs += [ nn.gradients (gpu_D_loss, self.dis.get_weights() ) ]

                    gpu_src_dst_loss_gvs += [ nn.gradients ( gpu_src_dst_loss, self.src_dst_trainable_weights ) ]
                    

                    gpu_var_loss  = nn.style_loss (gpu_pred_src_dst_1, gpu_pred_src_dst_0, gaussian_blur_radius=resolution//16, loss_weight=100)
                    gpu_var_loss += nn.style_loss (gpu_pred_src_dst_1, gpu_pred_src_dst_2, gaussian_blur_radius=resolution//16, loss_weight=100)
                    gpu_var_loss_gvs += [ nn.gradients ( gpu_var_loss, self.src_trainable_weights ) ]
            ...
            with tf.device (models_opt_device):
                if gpu_count == 1:
                    ...
                    var_loss_gv = nn.average_gv_list (gpu_var_loss_gvs)
