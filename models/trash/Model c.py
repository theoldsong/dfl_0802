            ...
            for gpu_id in range(gpu_count):
                with tf.device( f'/GPU:{gpu_id}' if len(devices) != 0 else f'/CPU:0' ):
                    ...
                    if 'df' in archi:
                        ...
                        #if 'tf' in archi:                            
                        gpu_pred_src_src_code = self.inter(self.encoder( gpu_pred_src_src )   ) 
                        ...
                        #if 'tf' in archi:                            
                        gpu_pred_src_src_code = self.encoder ( gpu_pred_src_src )                            
                        gpu_pred_src_src_inter_AB_code = self.inter_AB (gpu_pred_src_src_code)
                        gpu_pred_src_src_code = tf.concat([gpu_pred_src_src_inter_AB_code,gpu_pred_src_src_inter_AB_code], nn.conv2d_ch_axis  )
                    ...
                    #if 'tf' in archi:
                    #pass
                    #gpu_src_loss += tf.reduce_mean ( 10*tf.abs( gpu_src_code - gpu_pred_src_src_code ), axis=[1,2,3] )
 
                    #gpu_src_loss += tf.reduce_mean ( 300*tf.abs( gpu_src_code - gpu_dst_code ), axis=[1,2,3] )
                    
                    #d = tf.square(gpu_src_code - gpu_dst_code)
                    #d_sqrt = tf.sqrt(d)
                    #gpu_src_loss += tf.reduce_mean( tf.square(tf.maximum(0., tf.ones_like(d)*3.0 - d_sqrt)), axis=[1,2,3])
                    ...
                    gpu_dst_loss = tf.reduce_mean ( 1*nn.dssim(gpu_target_dst_masked_opt, gpu_pred_dst_dst_masked_opt, max_val=1.0, filter_size=int(resolution/11.6) ), axis=[1])
                    gpu_dst_loss += tf.reduce_mean ( 1*tf.square(  gpu_target_dst_masked_opt- gpu_pred_dst_dst_masked_opt ), axis=[1,2,3])
                    
                    if eyes_prio:
                        gpu_dst_loss += tf.reduce_mean ( 30*tf.abs ( gpu_target_dst*gpu_target_dstm_eyes - gpu_pred_dst_dst*gpu_target_dstm_eyes ), axis=[1,2,3])
                    
                    if learn_mask:
                        gpu_dst_loss += tf.reduce_mean ( 1*tf.square( gpu_target_dstm - gpu_pred_dst_dstm ),axis=[1,2,3] )
